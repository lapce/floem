    ...
    fn event_before_children(&mut self, cx: &mut EventCx, event: &Event) -> EventPropagation {
        ...
        let is_handled = match &event {
            ...
            Event::Pointer(PointerEvent::Down {
                ...
                self.id.request_layout();
                self.last_pointer_down = state.position;

                if state.count == 2 {
                    self.handle_double_click(state.position.x, state.position.y);
                } else if state.count == 3 {
                    self.handle_triple_click();
                } else {
                    self.cursor_glyph_idx =
                        self.get_box_position(state.position.x, state.position.y);
                    self.selection = None;
                }
                true
            }
