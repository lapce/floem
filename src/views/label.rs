        ...
        match event {
            Event::Pointer(PointerEvent::Down(PointerButtonEvent { state, .. })) => {
                if self.style.text_selectable() {
                    self.selection_range = None;
                    self.selection_state = SelectionState::Ready(state.point());
                    self.id.request_layout();
                }
            }
            Event::Pointer(PointerEvent::Move(pu)) => {
                if !self.style.text_selectable() {
                    ...
                    if start.distance(pu.current.point()).abs() > 2. {
                        self.selection_state = SelectionState::Selecting(start, pu.current.point());
