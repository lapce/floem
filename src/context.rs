            ...
            match &event {
                Event::Pointer(PointerEvent::Down(PointerButtonEvent {
                    ...
                    if pointer.is_primary_pointer() {
                        ...
                        if on_view {
                            ...
                            {
                                self.app_state.drag_start = Some((view_id, point));
                            }
                        }
                ...
                Event::Pointer(PointerEvent::Move(PointerUpdate { current, .. })) => {
                    ...
                    if self.app_state.draggable.contains(&view_id) {
                        ...
                        {
                            let offset = current.point() - *drag_start;
                ...
                Event::Pointer(PointerEvent::Up(PointerButtonEvent {
                    ...
                    if pointer.is_primary_pointer() {
                        let rect = view_id.get_size().unwrap_or_default().to_rect();
                        let on_view = rect.contains(state.point());
                            ...
                            self.app_state.dragging.as_mut().filter(|d| d.id == view_id)
                        {
                            ...
                            dragging.release_location = Some(state.point());
                            self.app_state.request_paint(view_id);
                            dragging_id.apply_event(&EventListener::DragEnd, &event);
                        }
                        ...
                        let viewport_event_position = {
                            let layout = view_state.borrow().layout_rect;
                            Point::new(layout.x0 + state.point().x, layout.y0 + state.point().y)
                        };
