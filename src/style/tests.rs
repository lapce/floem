...
fn style_override() {
    ...
    assert_eq!(style.get(PaddingLeft), Length::Pt(64.0));
    assert_eq!(style.get(PaddingBottom), Length::Pt(45.0)); // Should be preserved from style1
    ...
    assert_eq!(style.get(PaddingLeft), Length::Pt(64.0));
    assert_eq!(style.get(PaddingBottom), Length::Pt(45.));

    // Test that individual methods work correctly within a single style
    let style1 = Style::new().padding_left(32.0).padding_bottom(45.0);

    assert_eq!(style1.get(PaddingLeft), Length::Pt(32.0));
    assert_eq!(style1.get(PaddingBottom), Length::Pt(45.0)); // Both values are preserved in same style
