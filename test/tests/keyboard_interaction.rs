...
fn test_only_one_view_focused_at_time() {
    let root = TestRoot::new();
    let view1 = Empty::new().style(|s| s.size(50.0, 50.0).keyboard_navigable());
    let id1 = view1.view_id();

    let view2 = Empty::new().style(|s| s.size(50.0, 50.0).keyboard_navigable());
...
fn test_repeated_enter_emits_click() {
    ...
    let view = tracker
        ...
        .style(|s| s.size(100.0, 100.0).keyboard_navigable());
