...
pub fn img_view() -> impl IntoView {
    ...
    let ferris_png_data = Image::image_data(ferris_png);
    ...
    let sunflower_data = Image::image_data(sunflower);

    form((
        form_item(
            "PNG:",
            Image::new(ferris_png_data.clone()).style(|s| s.aspect_ratio(1.5)),
        ),
        form_item(
            "PNG(resized):",
            Image::new(ferris_png_data.clone()).style(|s| s.width(230.pt()).height(153.pt())),
        ),
        form_item(
            "SVG(from file):",
        ...
        form_item(
            "Image Fit",
            object_fit_position_picker(ferris_png_data.clone()),
        ),
        form_item("SVG(from string):", svg(svg_str).style(|s| s.width(100))),
        form_item("JPG:", Image::new(sunflower_data.clone())),
        form_item(
            "JPG(resized):",
            Image::new(sunflower_data.clone()).style(|s| s.width(320.pt()).height(490.pt())),
        ),
    ))
}

fn object_fit_position_picker(image_data: impl ImgSource) -> impl IntoView {
    ...
    let preview = Image::new(image_data).style(move |s| {
