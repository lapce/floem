...
pub fn img_view() -> impl IntoView {
    ...
    let ferris_png_data = Img::image_data(ferris_png);
    ...
    let sunflower_data = Img::image_data(sunflower);

    form((
        form_item(
            "PNG:",
            Img::new(ferris_png_data.clone()).style(|s| s.aspect_ratio(1.5)),
        ),
        form_item(
            "PNG(resized):",
            Img::new(ferris_png_data.clone()).style(|s| s.width(230.pt()).height(153.pt())),
        ),
        form_item(
            "SVG(from file):",
        ...
        form_item("JPG:", Img::new(sunflower_data.clone())),
        form_item(
            "JPG(resized):",
            Img::new(sunflower_data.clone()).style(|s| s.width(320.pt()).height(490.pt())),
        ),
    ))
}
...
fn object_fit_position_picker(image_data: peniko::ImageData) -> impl IntoView {
    ...
    let preview = Img::new(image_data).style(move |s| {
