use floem::{
    event::{Event,EventListener}, keyboard::{Key, NamedKey}, peniko::Color, reactive::{create_rw_signal, SignalGet, SignalUpdate}, views::{dyn_view, label, Decorators}, IntoView, View
};
...
pub fn dropped_file_view() -> impl IntoView {
    ...
    form({
        (
            form_item("File:".to_string(), 80.0, move || label(move || filename.get())),
